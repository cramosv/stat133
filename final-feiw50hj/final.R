...
github.name <- "cramosv"
email.address <- "cramosv@berkeley.edu"
...
cw4 <- ChickWeight[sample(c(1:578),200,replace=FALSE),]
...
abline(lm(mtcars$qsec~mtcars$hp)$coeff)
...
plot(mtcars$disp~mtcars$wt,ylab="Engine Displacement",xlab="Weight of the Car (lb/1000)",main="Engine Displacement vs. Car Weight",col=factor(mtcars$am))
...
plot(WorldBank$life.expectancy~WorldBank$fertility.rate,col=WorldBank$region,xlab="Fertility Rate",ylab="Life Expectancy",pch=".",main="Life Expectancy and Fertility Rates")
abline(v=2.1)
...
for.1960=WorldBank[WorldBank$year==1960,]
for.2013=WorldBank[WorldBank$year==2013,]
par(mfrow=c(1,2))
plot(for.1960$life.expectancy~for.1960$fertility.rate,xlab="Fertility Rate",ylab="Life Expectancy",pch=".",main="Life Expectancy and Fertility Rates")
abline(v=2.1)
plot(for.2013$life.expectancy~for.2013$fertility.rate,xlab="Fertility Rate",ylab="Life Expectancy",pch=".",main="Life Expectancy and Fertility Rates")  
abline(v=2.1)
par(mfrow=c(1,1))
...
lenders=WorldBank[WorldBank$lending=="IDA",]

h=seq(from=0,to=7000,by=250)

hist(WorldBank[WorldBank$lending=="IDA","GDP.per.capita.Current.USD"],breaks=h,xlab="GDP per Capita",ylab="Lending Frequency",main="IDA Lending Rate Borrowing Countries")
...
#     we standardize by subtracting the mean of the column/row and dividing by the sd of the column/row
 ...
 standardizeVar <- function(m, cols=TRUE){
   ...
   if(cols==TRUE){
     ...
     mean=apply(m,2,mean)
     sd.=apply(m,2,sd)
     first[,i]=(m[,i]-mean[i])/sd.[i]}}
   else{
     for (i in 1:nrow(m)){
     mean2=apply(m,1,mean)
     sd.2=apply(m,2,sd)
      first[i,]=(m[i,]-mean2[i])/sd.2[i]
   }}
   return(first)}
...
##   <your code here>
  emptyvec=c(rep(k,0))
  difference=c(1:k)
  together=c(cases,controls)
  for (i in 1:k){

  new.cases=sample(length(cases),difference,replace=FALSE)
  new.controls=together[together[i]!=new.cases]
  emptyvec[i]=mean(new.cases)-mean(new.controls)}
  return(emptyvec)
}
...
for (i in 1:k){
  ...
  players[[i]]=sampling
  players=sapply(players,sort)
  players=apply(players,2,function(x)x-winners)
  for (j in 1:ncol(players)){
    if (all(players[,j]==winners)){
      m=m+1 
    }
    else{
    }
